...
public class FusionCard : MonoBehaviour
{
    ...
    void ActivateDraw(FusionHands p_hands, FusionDeck p_deck, Vector3 location, Vector3 scale, int amount)
    {
    ...
    public int GetCostByCondition(FusionPlayer player)
    {
        if (GetIfNormalCard())
        {
            return GetCost();
    ...
    public int GetAmountByCondition(FusionPlayer player, Situation.PlayingSituation situation)
    {
    ...
    public bool MetRestrictions(FusionPlayer player)
    {
    ...
    //カードを出したときの効果
    public void Effect(FusionPlayer player, FusionHands p_hands, FusionDeck p_deck, FusionPlayer enemy, FusionHands e_hands, FusionDeck e_deck, Vector3 location, Vector3 scale, Situation situation)
    {
        if (GetIfNormalCard())
        {
    ...
    void EffectList(string effect_text, FusionPlayer player, FusionHands p_hands, FusionDeck p_deck, FusionPlayer enemy, FusionHands e_hands, FusionDeck e_deck, Vector3 location, Vector3 scale, int amount, int bonus_amount, Situation situation, bool situation_match = false)
    {
        switch (effect_text)
        {
    ...
    public string GetEffectByCondition(FusionPlayer player)
    {
        if (GetCardType() != CardType.Normal)
        {
            if (GetCardType() == CardType.HopeAndDespair)
