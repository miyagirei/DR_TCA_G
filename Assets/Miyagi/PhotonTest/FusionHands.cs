...
using UnityEngine;
using System.IO;
public class FusionHands : MonoBehaviour
{
    string _name = "";
    public void SetName(string name) => _name = name;
    public string GetName() => _name;
    ParameterData _parameter_data_controller;
    [SerializeField] List<FusionCard> _hands_card = new List<FusionCard>();
    ...
    //カーソル上にあるデッキを取得する
    public FusionDeck PickDrawDeck(FusionDeck mine)
    {
        ...
        FusionDeck selectObj = hit2d.transform.gameObject.GetComponent<FusionDeck>();
        if (selectObj == null)
        {
            return null;
        }

        if (selectObj.TryGetComponent<FusionDeck>(out FusionDeck deck))
        {
            if (mine != deck)
            {
                return null;
    ...
    public void CreateCard(Card origin, Vector3 pos, Vector3 scale, FusionDeck deck)
    {
        ...
        FusionCard new_card = card_obj.GetComponent<FusionCard>();
    ...
    public FusionCard IsPlayedCard(FusionPlayer player)
    {
        foreach (FusionCard card in _hands_card)
        {
            if (!card.GetPlayed())
            {
                continue;
    ...
    //カードを捨てる時にリストから排除する
    public void TrashCard(FusionCard card)
    {
    ...
    public FusionCard CheckMostExpensiveCardYouCanPay(FusionPlayer player)
    {
        ...
        FusionCard most_card = new FusionCard();
    ...
    public void ShowAvailableCards(FusionCard exception)
    {
        for (int i = 0; i < _hands_card.Count; i++)
        {
    ...
    public void ResetHandCards(int cards, FusionDeck deck, Vector3 pos, Vector3 scale)
    {
        Debug.Log("ResetCard");
        ...
        if (cards < _hands_card.Count)
        {
            ...
            {
                FusionCard card = _hands_card[Random.Range(0, _hands_card.Count)];
    ...
    public void TrashCardRandom(int count)
    {
        ...
        {
            FusionCard card = _hands_card[Random.Range(0, _hands_card.Count - 1)];
            TrashCard(card);
        }
    }
